...
def get_overcrediting(condition=None, percentage=True, display=False):
    ...
    total_percentage = list(
        np.percentile(total, [5, 50, 95])
        / np.sum([[x for x in data if x["id"] == key][0]["arbocs"]["issuance"] for key in keys])
    )
    total_arbocs = list(np.percentile(total, [5, 50, 95]))
    total_alt_slag = list(np.percentile(total_alt, [5, 50, 95]))

    return {"percent": total_percentage, "arbocs": total_arbocs, "alt_slag": total_alt_slag}
