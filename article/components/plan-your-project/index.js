...
const Index = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 15 * 8; i++) {
      if (active[i]) {
        const region = whichRegion(positions[i])
        currentTotal += Math.max(increments[categories[i]] - baseline[region], 0)
      }
    }
  ...
  return <Box>
    ...
    <Box sx={{
      ...
      <Radio value={1} label={'Scenario 1'} current={scenario} set={setScenario} color='white'/>
      <Radio value={2} label={'Scenario 2'} current={scenario} set={setScenario} color='white'/>
    </Box>
    <Box sx={{
      float: 'right',
      mr: ['50px'],
