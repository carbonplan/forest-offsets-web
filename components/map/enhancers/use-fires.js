...
function useFires(map, fires) {
  ...
  useEffect(() => {
  	if (fires) {
    	...
    	map.setPaintProperty('projects-center', 'circle-color', mix(0.5, primary, background))
    	...
    	map.setPaintProperty('projects-center', 'circle-color', mix(0.5, green, background))
  	}
  }, [map, fires, theme])
}
